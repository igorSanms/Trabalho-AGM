...
def gerar_grafico(tipo, x, y, erro, cor, titulo):
    ...
    nome_imagem = f"resultados/grafico_prim_{tipo}.png"
    plt.savefig(nome_imagem, dpi=300, bbox_inches='tight')
    print(f"Gráfico salvo: {nome_imagem}")
    plt.close()

if __name__ == "__main__":
    dados = carregar_dados("resultados/resultados_prim.csv")
    gerar_grafico('esparso', dados['esparso']['x'], dados['esparso']['y'], dados['esparso']['erro'], 'blue', 'Desempenho Prim - Grafos Esparsos')
    gerar_grafico('denso', dados['denso']['x'], dados['denso']['y'], dados['denso']['erro'], 'red', 'Desempenho Prim - Grafos Densos')
    gerar_grafico('geometrico', dados['geometrico']['x'], dados['geometrico']['y'], dados['geometrico']['erro'], 'green', 'Desempenho Prim - Grafos Geométricos')  
